  ...
  <q-page padding>
    ...
    <div>
      <q-table
        ...
        :loading="loading"
        :filter="filter"
        @request="getDasherList"
        binary-state-sort
      >
        <template v-slot:top-left>
          <q-btn
            label="Register dasher"
            :to="{ path: '/admin/users/dasher/register'}"
            color="primary"
            icon="add_box"
          />
        </template>
        ...
        <template v-slot:body-cell-action="cellProperties">
          <q-td :props="cellProperties">
            <q-btn
              ...
              color="primary"
              icon="search"
              :to="{ path: '/admin/users/dasher/view/' + cellProperties.value }"
            />
          </q-td>
        </template>
      </q-table>
...
export default {
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'vehicle_rank', label: 'VEHICLE RANK', align: 'center', field: 'vehicle_rank', sortable: true },
  ...
  methods: {
    getDasherList () {
      ...
      this.loading = true
      this.$store.dispatch('adminDasherModule/getDasherList', {
        startRow: startRow,
      ...
      this.$store.dispatch('adminDasherModule/getDasherListCount')
        .then(response => {
          ...
          response.forEach(data => {
            ...
            if (data['vehicle_rank'].includes(filter)) {
              ++count
            }
