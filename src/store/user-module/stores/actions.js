...
export const getStoreCategories = (context, payload) => {
  ...
  return new Promise((resolve, reject) => {
    axios
      .get("/store_categories?id=" + payload.id)
      .then(response => {
        resolve(response);
      })
      .catch(error => {
...
export const getStoreProductsByCategory = (context, payload) => {
  ...
  return new Promise((resolve, reject) => {
    axios
      .get(
        "/store_products_by_category?category_id=" +
          payload.categoryId +
          "&merchant_id=" +
          payload.merchantId
      )
...
export const getStoreBanner = (context, payload) => {
  ...
  return new Promise((resolve, reject) => {
    axios
      .get("/store_getBanner?merchant_id=" + payload.merchant_id)
      .then(response => {
        resolve(response);
      })
      .catch(error => {
...
export const addToCart = (context, payload) => {
  const cart = LocalStorage.has("cart")
...
export const updateCart = (context, payload) => {
  var cart = LocalStorage.has("cart")
    ? JSON.parse(LocalStorage.getItem("cart"))
    : [];
  cart = [];
  cart = payload.cart;
...
export const placeOrder = async (context, payload) => {
  ...
  let merchLatLong = await getMerchantLatLong(
    context.rootState.loginModule.token
  );
